...
class DecodeUnitIo(implicit p: Parameters) extends BoomBundle
{
  ...
  // Decode unit must recieve the frontend taint
  // added by mofadiheh for taint
  val front_taint = Input(Bool())
}
...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  // all insts at Decode stage are tainted according to the frontend taint
  // added by mofadiheh for taint
  uop.taint := io.front_taint
