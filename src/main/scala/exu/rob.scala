...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    for (i <- 0 until numRobRows) {
      ...
      when (IsKilledByBranch(io.brupdate, br_mask))
      {
		...
		rob_uop(i).taint   := GetNewImplicitTaint(io.brupdate, rob_uop(i))
  ...
  // the taint should be updated too
  // added by mofadiheh for taint
  r_xcpt_uop.taint   := GetNewImplicitTaint(io.brupdate, next_xcpt_uop)
  ...
  // -----------------------------------------------
  // added by tojauch for PNR debug purposes

  val head_to_pnr_diff = rob_pnr_idx - rob_head_idx
  dontTouch(head_to_pnr_diff)

  val max_pnr_diff = RegInit(0.U)

  when (head_to_pnr_diff > max_pnr_diff) {
    max_pnr_diff := head_to_pnr_diff
  }

  dontTouch(max_pnr_diff)

  val pnr_diff_gr_five = RegInit(false.B)

  when (head_to_pnr_diff > 5.U) {
    pnr_diff_gr_five := true.B
  } .otherwise {
    pnr_diff_gr_five := false.B
  }

  dontTouch(pnr_diff_gr_five)

  val pnr_diff_gr_ten = RegInit(false.B)

  when (head_to_pnr_diff > 10.U) {
    pnr_diff_gr_ten := true.B
  } .otherwise {
    pnr_diff_gr_ten := false.B
  }

  dontTouch(pnr_diff_gr_ten)
