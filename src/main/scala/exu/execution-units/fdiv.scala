  ...
  with tile.HasFPUParameters
{
  ...
  r_buffer_val := !IsKilledByBranch(io.brupdate, r_buffer_req.uop, 4.U) && !io.req.bits.kill && r_buffer_val
	...
	r_buffer_req.uop.taint   := GetNewImplicitTaint(io.brupdate, r_buffer_req.uop)
  ...
  when (io.req.valid && !IsKilledByBranch(io.brupdate, io.req.bits.uop, 4.U) && !io.req.bits.kill) {
	...
	// the taint should be updated too
	//added by mofadiheh for taint
	r_buffer_req.uop.taint   := GetNewImplicitTaint(io.brupdate, io.req.bits.uop)
  ...
  // the taint should be updated too
  //added by mofadiheh for taint
  r_divsqrt_uop.taint   := GetNewImplicitTaint(io.brupdate, r_divsqrt_uop)
  ...
  when (may_fire_input && divsqrt_ready) {
	...
	// the taint should be updated too
	//added by mofadiheh for taint
	r_divsqrt_uop.taint   := GetNewImplicitTaint(io.brupdate, r_buffer_req.uop)
  ...
  // the taint should be updated too
  //added by mofadiheh for taint
  r_out_uop.taint   := GetNewImplicitTaint(io.brupdate, r_out_uop)
  ...
  when (divsqrt.io.outValid_div || divsqrt.io.outValid_sqrt) {
	...
	// the taint should be updated too
	//added by mofadiheh for taint
	r_out_uop.taint   := GetNewImplicitTaint(io.brupdate, r_divsqrt_uop)
  ...
  io.resp.valid := r_out_val && !IsKilledByBranch(io.brupdate, r_out_uop)//&& !IsKilledByBranch(io.brupdate, r_out_uop, 4.U)
  ...
  // re-add possibility for it to be killed by a branch
  ...
  // the taint should be updated too
  //added by mofadiheh for taint
  io.resp.bits.fflags.bits.uop.taint   := GetNewImplicitTaint(io.brupdate, r_out_uop)
