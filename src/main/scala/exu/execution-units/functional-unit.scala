...
class BrUpdateInfo(implicit p: Parameters) extends BoomBundle
{
  ...
  // we use this bundle to broadcast the output of the security monitor
  //added by mofadiheh for taint
  val sec_alert = new SecurityAlert
}
...
class BrUpdateMasks(implicit p: Parameters) extends BoomBundle
{
  ...
  //added by mofadiheh for taint
  // New mask used for implicit taint propagation to backend
  val taint_mask = UInt(maxBrCount.W)
  // We use this infrastructure also for taint clear
  // For this reason we need to broadcast PNR and rob_head
  val rob_pnr_idx  = UInt(robAddrSz.W)
  val rob_head_idx = UInt(robAddrSz.W)
}
    ...
    needsFcsr = needsFcsr)
{
  ...
  if (numStages > 0) {
    ...
    r_valids(0) := io.req.valid && !IsKilledByBranch(io.brupdate, io.req.bits.uop, alert_identifier) && !io.req.bits.kill // only the first stage can be aborted; modified by mofadiheh for taint
    r_uops(0)   := io.req.bits.uop
    r_uops(0).br_mask := GetNewBrMask(io.brupdate, io.req.bits.uop)

	// Implicit Taint must be updated
	// added by mofadiheh for taint
	r_uops(0).taint   := GetNewImplicitTaint(io.brupdate, io.req.bits.uop)
    ...
    for (i <- 1 until numStages) {
	  ...
	  // Implicit Taint must be updated
	  // added by mofadiheh for taint
	  r_uops(i).taint   := GetNewImplicitTaint(io.brupdate, r_uops(i-1))
	...
	// Implicit Taint must be updated
	// added by mofadiheh for taint
	io.resp.bits.uop.taint   := GetNewImplicitTaint(io.brupdate, r_uops(numStages-1))
	...
	// Implicit Taint must be updated
	// added by mofadiheh for taint
	io.resp.bits.uop.taint   := GetNewImplicitTaint(io.brupdate, io.req.bits.uop)
  }
}
 ...
 */
@chiselName
class ALUUnit(isJmpUnit: Boolean = false, numStages: Int = 1, dataWidth: Int, alert_identifier: UInt)(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
    ...
    dataWidth = dataWidth,
    isJmpUnit = isJmpUnit,
	alert_identifier = alert_identifier) // added by mofadiheh for taint
  with boom.ifu.HasBoomFrontendParameters
{
  ...
  when (io.req.bits.kill || IsKilledByBranch(io.brupdate, uop, alert_identifier)) { // modified by mofadiheh for taint
    killed := true.B
  }
...
//   io.resp.bits.data := reg_data
  ...
  r_val (0) := io.req.valid && !IsKilledByBranch(io.brupdate, uop, alert_identifier) // The second clause added by mofadiheh to accomodate aborted uops by security monitor
    ...
    dataWidth = dataWidth)
{
  ...
  when (io.req.fire) {
	...
	// Implicit Taint must be updated
	// added by mofadiheh for taint
	r_uop.taint   := GetNewImplicitTaint(io.brupdate, r_uop)
