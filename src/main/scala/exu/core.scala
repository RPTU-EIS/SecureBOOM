  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  // Input comming from security monitor
  // consists of the alert and rob_idx of aborted instructions
  // added by mofadiheh for taint
  val sec_alert = Input(new SecurityAlert())
  val sec_monitor_signals = Output(new SecurityMonitorSignals())
...
// additional variables to extract the parameters for numTotalWakeupPorts
  val fpIssueParams_temp = issueParams.find(_.iqType == IQT_FP.litValue).get
  val numLlPorts_temp = memWidth
  val numFpWakeupPorts_temp = fpIssueParams_temp.issueWidth + numLlPorts_temp


  // Meanwhile, the FP pipeline holds the FP issue window, FP regfile, and FP arithmetic units.
  var fp_pipeline: FpPipeline = null
  if (usingFPU) fp_pipeline = Module(new FpPipeline(numIrfWritePorts + numFpWakeupPorts_temp))
  ...
  if (usingFPU) {
    ...
    fp_pipeline.io.wb_resps  := DontCare
  }
  ...
  val mem_iss_unit     = Module(new IssueUnitCollapsing(memIssueParam, numIntIssueWakeupPorts, numIrfWritePorts + numFpWakeupPorts))
  mem_iss_unit.suggestName("mem_issue_unit")
  val int_iss_unit     = Module(new IssueUnitCollapsing(intIssueParam, numIntIssueWakeupPorts, numIrfWritePorts + numFpWakeupPorts))
  ...
  val iregister_read   = Module(new RegisterRead(
                           ...
                           xLen,
													 true.B)) // last param added by Mo, isInteger = True
  ...
  // Set pnr broadcast signals
  // added by mofadiheh for taint
  b1.rob_pnr_idx  := rob.io.rob_pnr_idx
  b1.rob_head_idx := rob.io.rob_head_idx


// --------------------
// This part is for implicit taints
// For the time being it is disabled since all the tainted branches are aborted
// --------------------

	// Set the taint mask
//  b1.taint_mask := brinfos.map(x => (x.valid && x.uop.taint)<< x.uop.br_tag).reduce(_|_)

	// To disable implicit taint
  b1.taint_mask := 0.U



	// Set the taint for Frontend
	val curr_front_taint = Wire(new Bool)
	val front_taint_reg = Reg(new Bool)

  // Find the youngest tainted br
  var taint_val = false.B
  var curr_younger_tainted = brinfos(0).uop.rob_idx

  for (b <- brinfos) {
    val use_this_tainted = !taint_val ||
    b.valid && b.uop.taint && !IsOlder(b.uop.rob_idx, curr_younger_tainted, rob.io.rob_head_idx)

    taint_val = taint_val || (b.valid && b.uop.taint)
    curr_younger_tainted = Mux(use_this_tainted, b.uop.rob_idx, curr_younger_tainted)
  }


	val youngest_tainted = Wire(UInt(robAddrSz.W))
	val youngest_tainted_br_reg = Reg(UInt(robAddrSz.W))

	when (taint_val && !front_taint_reg) {
		youngest_tainted := curr_younger_tainted
	} .elsewhen(!taint_val) {
		youngest_tainted := youngest_tainted_br_reg
	} .otherwise {
		youngest_tainted := Mux( !IsOlder(curr_younger_tainted, youngest_tainted_br_reg, rob.io.rob_head_idx) , curr_younger_tainted , youngest_tainted_br_reg )
	}

//	youngest_tainted := Mux( taint_val && !IsOlder(curr_younger_tainted, youngest_tainted_br_reg, rob.io.rob_head_idx) , curr_younger_tainted , youngest_tainted_br_reg )
	youngest_tainted_br_reg := youngest_tainted

// ------------------
// End of implicit taint
// ------------------
...
// --------------------
// This part is for implicit taints
// For the time being it is disabled since all the tainted branches are aborted (disabled at the decode io input
// added by mofadiheh for taint
// --------------------


  // We need a taint clear logic: in addition to pnr, we should also monitor branch mispredict for taint clear
  // if the oldest mispredict is not tainted, the frontend taint must be cleared
  // if youngest tainted br is past PNR, the taint must be cleared
  // frontend gets tainted in both mis and correct prediction cases
    when (mispredict_val && oldest_mispredict.uop.taint === false.B) {
	  curr_front_taint := false.B
    } .elsewhen( IsOlder(youngest_tainted, rob.io.rob_pnr_idx, rob.io.rob_head_idx) || youngest_tainted === rob.io.rob_pnr_idx ) { // what about pnr == youngest
	  curr_front_taint := false.B
	} .otherwise {
	  curr_front_taint := front_taint_reg || ( brinfos.map(x => (x.valid && x.uop.taint)).reduce(_|_) )
	}
  // curr_front_taint is fed to decode unit
  front_taint_reg := curr_front_taint
...
// ------------------
  ...
  assert (!((brupdate.b1.mispredict_mask =/= 0.U || brupdate.b2.mispredict)
	...
	// connect the brupdate broadcast network to the security monitor
	// added by mofadiheh for taint
	brupdate.sec_alert := io.sec_alert
  ...
  if (usingFPU) {
    fp_pipeline.io.brupdate := brupdate
		// inputs for untainting register file
		// added by mofadiheh for taint
		fp_pipeline.io.rob_pnr_idx  := rob.io.rob_pnr_idx
		fp_pipeline.io.rob_head_idx := rob.io.rob_head_idx
		fp_pipeline.io.commit       := rob.io.commit
...
// added by mofadiheh for taint
// --------------------
  // Frontend taint must be given to decode unit
  //decode_units(w).io.front_taint     := curr_front_taint

  decode_units(w).io.front_taint     := false.B
...
//---------------------
  ...
  //-------------------------------------------------------------
    // added by mofadiheh for taint
	// setting the initial yrot for every instruction
	// by default yrot is initial set equal to the instruction rob_idx
	// later if the taint is set, it will be changed in the register read stage
	// in case of implicit taint (currently disabled) this should also change to avoid
	// overconservative untainting

  for (w <- 0 until coreWidth) {
    // note: this assumes uops haven't been shifted - there's a 1:1 match between PC's LSBs and "w" here
    // (thus the LSB of the rob_idx gives part of the PC)
    if (coreWidth == 1) {
      dis_uops(w).yrot := rob.io.rob_tail_idx
    } else {
      dis_uops(w).yrot := Cat(rob.io.rob_tail_idx >> log2Ceil(coreWidth).U,
                               w.U(log2Ceil(coreWidth).W))
    }
  }
...
//   "CSRFile is being written to spuriously.")
  ...
  //-------------------------------------------------------------
	...
	// inputs to regfile for untainting
	// added by mofadiheh for taint
  ...
  for (i <- 0 until exe_units.length) {
    if (exe_units(i).writesIrf) {
	  ...
	  val wbtaint = wbresp.bits.uop.taint // added by mofadiheh for taint
      ...
      if (exe_units(i).hasCSR) {
		...
		iregfile.io.write_ports(w_cnt).bits.taint := Mux(wbReadsCSR, false.B, wbtaint)  // added by mofadiheh for taint
      } else {
        iregfile.io.write_ports(w_cnt).bits.data := wbdata
		iregfile.io.write_ports(w_cnt).bits.taint := wbtaint // added by mofadiheh for taint
      }
  ...
  // connect writeback signals to issue stations too
  // added by mofadiheh for taint
  int_iss_unit.io.wb_resps(0).valid  := ll_wbarb.io.out.valid && !(ll_uop.uses_stq && !ll_uop.is_amo)
  int_iss_unit.io.wb_resps(0).bits   <> ll_wbarb.io.out.bits
  mem_iss_unit.io.wb_resps(0).valid  := ll_wbarb.io.out.valid && !(ll_uop.uses_stq && !ll_uop.is_amo)
  mem_iss_unit.io.wb_resps(0).bits   <> ll_wbarb.io.out.bits
  fp_pipeline.io.wb_resps(0).valid  := ll_wbarb.io.out.valid && !(ll_uop.uses_stq && !ll_uop.is_amo)
  fp_pipeline.io.wb_resps(0).bits   <> ll_wbarb.io.out.bits
...
// added by mofadiheh for taint
    int_iss_unit.io.wb_resps(cnt).valid := mem_resps(i).valid && !(mem_uop.uses_stq && !mem_uop.is_amo)
    int_iss_unit.io.wb_resps(cnt).bits  := mem_resps(i).bits
    mem_iss_unit.io.wb_resps(cnt).valid := mem_resps(i).valid && !(mem_uop.uses_stq && !mem_uop.is_amo)
    mem_iss_unit.io.wb_resps(cnt).bits  := mem_resps(i).bits
    fp_pipeline.io.wb_resps(cnt).valid := mem_resps(i).valid && !(mem_uop.uses_stq && !mem_uop.is_amo)
    fp_pipeline.io.wb_resps(cnt).bits  := mem_resps(i).bits
 ...
 // added by mofadiheh for taint
	  int_iss_unit.io.wb_resps(cnt).valid := resp.valid && !(wb_uop.uses_stq && !wb_uop.is_amo)
      int_iss_unit.io.wb_resps(cnt).bits  <> resp.bits
	   mem_iss_unit.io.wb_resps(cnt).valid := resp.valid && !(wb_uop.uses_stq && !wb_uop.is_amo)
      mem_iss_unit.io.wb_resps(cnt).bits  <> resp.bits
	  fp_pipeline.io.wb_resps(cnt).valid := resp.valid && !(wb_uop.uses_stq && !wb_uop.is_amo)
      fp_pipeline.io.wb_resps(cnt).bits  <> resp.bits
  ...
  if (usingFPU) {
    for ((wdata, wakeup) <- fp_pipeline.io.debug_wb_wdata zip fp_pipeline.io.wakeups) {
      ...
      int_iss_unit.io.wb_resps(cnt) <> wakeup
      mem_iss_unit.io.wb_resps(cnt) <> wakeup
      fp_pipeline.io.wb_resps(cnt) <> wakeup
  ...
  // Connect LSU clear busy bit to issue stations
  // added by mofadiheh for taint
  int_iss_unit.io.lsu_clr_bsy := io.lsu.clr_bsy
  mem_iss_unit.io.lsu_clr_bsy := io.lsu.clr_bsy
  fp_pipeline.io.lsu_clr_bsy := io.lsu.clr_bsy
