...
class BoomTileModuleImp(outer: BoomTile) extends BaseTileModuleImp(outer){
  ...
  //added by mofadiheh for taint
	val secMonitor = Module(new SecurityMonitor()(outer.p))

	core.io.sec_alert <> secMonitor.io.sec_alert
  secMonitor.io.sec_mon_inputs <> core.io.sec_monitor_signals
	/*when (core.io.interrupts.mtip) {
		secMonitor.io.temp := 1023.U
	} .otherwise {
		secMonitor.io.temp := 512.U
	}*/
}
