...
//
// Additional source code by Tobias Jauch, Mohammad Rahmani Fadiheh, Philipp Schmitz and Alex Wezel: 11/01/2023 (Meltdown Fix + STT Spectre)
...
import boom.util.{BoolToChar, AgePriorityEncoder, IsKilledByBranch, GetNewBrMask, GetNewImplicitTaint, WrapInc, IsOlder, UpdateBrMask}
  ...
  with rocket.HasL1HellaCacheParameters
{
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val can_fire_sta_retry     = widthMap(w =>
                               ( stq_retry_e.valid                            &&
                                 ...
                                 stq_retry_e.bits.addr_is_virtual             &&
															   !stq_retry_e.bits.uop.taint                  && // added by tojauch STT bug fix fix
                                 (w == memWidth-1).B                          &&
                                 RegNext(dtlb.io.miss_rdy)                    &&
                                 !(widthMap(i => (i != w).B               &&
  ...
  r_xcpt.uop.taint   := GetNewImplicitTaint(io.core.brupdate, xcpt_uop) // added by mofadiheh for taint
    ...
    when (io.dmem.resp(w).valid)
    {
        ...
        .elsewhen (io.dmem.resp(w).bits.uop.uses_stq)
      {
        ...
        when (io.dmem.resp(w).bits.uop.is_amo) {
          ...
          // added by mofadiheh for taint
		  // This is the first taint set, the result of any speculative AMO is marked as tainted
		  // Is it even possible?
		  // for the case rob_idx == pnr: should we set the taint or not, assuming we do the separate meltdown fix, we dont need to
		  when (!IsOlder(stq(io.dmem.resp(w).bits.uop.stq_idx).bits.uop.rob_idx, io.core.brupdate.b1.rob_pnr_idx, io.core.brupdate.b1.rob_head_idx) &&
			stq(io.dmem.resp(w).bits.uop.stq_idx).bits.uop.rob_idx =/= io.core.brupdate.b1.rob_pnr_idx)
		  {
			io.core.exe(w).iresp.bits.uop.taint := true.B
			io.core.exe(w).fresp.bits.uop.taint := true.B
		  }
