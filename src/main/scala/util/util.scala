...
// New IsKilledByBranch function which also include being killed by security monitor
...
// new function for updating taint on implicit taint propagation
// whenever the getNewBrMask is used, this function must be used too
// added by mofadiheh for taint

object GetNewImplicitTaint
{
   def apply(brupdate: BrUpdateInfo, uop: MicroOp): Bool = {
...
object UpdateBrMask
{
  def apply(brupdate: BrUpdateInfo, uop: MicroOp): MicroOp = {
	...
	// also update the taint bit in case of implicit taint propagation
	out.taint := GetNewImplicitTaint(brupdate,uop) // added by mofadiheh for taint
    out
  }
  def apply[T <: boom.common.HasBoomUOP](brupdate: BrUpdateInfo, bundle: T): T = {
	...
	// also update the taint bit in case of implicit taint propagation
	out.uop.taint := GetNewImplicitTaint(brupdate,bundle.uop) // added by mofadiheh for taint
    out
  }
  def apply[T <: boom.common.HasBoomUOP](brupdate: BrUpdateInfo, bundle: Valid[T]): Valid[T] = {
	...
	// also update the taint bit in case of implicit taint propagation
	out.bits.uop.taint := GetNewImplicitTaint(brupdate, bundle.bits.uop) // added by mofadiheh for taint
    out
  }
}
  ...
  with boom.common.HasBoomCoreParameters
{
  ...
  for (i <- 0 until entries) {
    ...
    when (valids(i)) {
	  ...
	  uops(i).taint := GetNewImplicitTaint(io.brupdate, uops(i)) // added by mofadiheh for taint
	  // ----
  ...
  when (do_enq) {
	...
	// the taint should be updated too
	uops(enq_ptr.value).taint 	:= GetNewImplicitTaint(io.brupdate, io.enq.bits.uop) // added by mofadiheh for taint
  ...
  // the taint should be updated too
  io.deq.bits.uop.taint := GetNewImplicitTaint(io.brupdate, out.uop) // added by mofadiheh for taint
  ...
  if (flow) {
    when (io.empty) {
	  ...
	  // the taint should be updated too
	  io.deq.bits.uop.taint := GetNewImplicitTaint(io.brupdate, io.enq.bits.uop) // added by mofadiheh for taint
